 ...
 *	Package: Prime Cab
 *  File For Contact Us Page Contact Form For Sending Email
 */
...
function sendEmailMsg(){
	...
	if(!empty($_POST['formData'])){
		...
		$emailAdd= filter_var($formData['email'], FILTER_SANITIZE_EMAIL);
		...
		if(!empty($name) && !empty($emailAdd) && !empty($subject) && !empty($message)){
